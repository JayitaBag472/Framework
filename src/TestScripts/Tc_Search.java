...
Library.Utility ul= new Library.Utility();
	...
	{
		logger=report.startTest("To test search feature");
		
		dr.get(config_File.url);
		//Click on Login button
		if(dr.findElements(By.xpath(p.getProperty("search"))).size()>0)
		{
			String img=logger.addScreenCapture(lib.captureScreenShot(dr, ul.timeStamp()));
			logger.log(LogStatus.PASS, "Search field is displayed", img);
			dr.findElement(By.xpath(p.getProperty("search"))).sendKeys("Butterfly");
			dr.findElement(By.xpath(p.getProperty("search"))).sendKeys(Keys.ENTER);
			
			img=logger.addScreenCapture(lib.captureScreenShot(dr, ul.timeStamp()));
			logger.log(LogStatus.INFO, "Search data", img);
			
		}else
		{
			String img=logger.addScreenCapture(lib.captureScreenShot(dr, ul.timeStamp()));
			logger.log(LogStatus.FAIL, "Search field is not displayed", img);
